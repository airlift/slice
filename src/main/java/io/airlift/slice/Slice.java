        ...
        implements Comparable<Slice>
{
    ...
    Slice(byte[] base)
    {
        this(MemorySegment.ofArray(base));
    }

    Slice(MemorySegment segment)
    {
        this(segment, 0, toIntExact(segment.byteSize()));
    }
    ...
    {
        this(MemorySegment.ofArray(base), offset, length);
    }

    Slice(MemorySegment segment, int baseOffset, int length)
    {
        requireNonNull(segment, "segment is null");
        checkArgument(segment.byteSize() > 0, "Empty memory segment: " + segment);
        this.segment = segment.asSlice(baseOffset, length);
    ...
    public int length()
    {
        return toIntExact(segment.byteSize());
    }
    ...
    {
        return INSTANCE_SIZE + SizeOf.sizeOf(segment);
    }
    ...
    {
        return byteArray().length == segment.byteSize();
    }
    ...
    public Slice copy()
    {
        ...
        MemorySegment copy = MemorySegment.ofArray(new byte[length()]);
        MemorySegment.copy(segment, 0, copy, 0, length());
    ...
    public Slice copy(int index, int length)
    {
        ...
        MemorySegment copy = MemorySegment.ofArray(new byte[length]);
        MemorySegment.copy(segment, index, copy, 0, length);
        return new Slice(copy);
    }
